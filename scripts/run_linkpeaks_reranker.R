...
option_list <- list(
  ...
  make_option(c("--candidate-filter"), type = "character", default = "positive_score",
              help = "LinkPeaks candidate filter: none, positive_score, or significant [default %default]"),
...
build_gene_tss_table <- function() {
  ...
  # Keep all transcript TSSs. Distance is later collapsed to the nearest TSS
  # per peak-gene pair, which is safer for genes with alternative promoters.
...
add_min_tss_distance <- function(dt, gene_coord_df, distance_d0) {
  dt <- copy(dt)
  dt[, row_id__ := .I]
  ...
  expanded <- merge(dt, peak_df, by = "peak", all.x = TRUE, sort = FALSE)
  expanded <- merge(
    expanded,
    gene_coord_df[, .(gene, gene_chr, tss, tx_id, tx_biotype)],
    by = "gene",
    all.x = TRUE,
    sort = FALSE,
  ...
  expanded[, distance_tmp__ := ifelse(
  ...
  best <- expanded[, {
    finite_idx <- which(is.finite(distance_tmp__))
    if (length(finite_idx) > 0) {
      .SD[finite_idx[which.min(distance_tmp__[finite_idx])]]
    } else {
      .SD[1]
    }
  }, by = row_id__]

  best[, distance_bp := distance_tmp__]
  best[, distance_score := ifelse(
    is.finite(distance_bp),
    1 / (1 + (distance_bp / distance_d0)^2),
    0
  )]

  setorder(best, row_id__)
  best[, c("row_id__", "distance_tmp__") := NULL]
  best
}
...
lsi_end <- min(opt$lsi_dims_end, lsi_max)

if (length(pca_dims) < 2) {
  stop("Not enough PCA dimensions available for WNN. Available PCA dims: ", pca_max)
}

if (opt$lsi_dims_start > lsi_end) {
  stop(
    "Not enough LSI dimensions available for WNN. Requested start dim ",
    opt$lsi_dims_start, ", available LSI dims: ", lsi_max
  )
}

lsi_dims <- seq.int(opt$lsi_dims_start, lsi_end)

if (length(lsi_dims) < 2) {
  stop("Not enough LSI dimensions available for WNN. Using dims: ", paste(lsi_dims, collapse = ","))
}

msg("Using PCA dims 1:%d and LSI dims %d:%d", max(pca_dims), min(lsi_dims), max(lsi_dims))
...
msg("Number of peak-gene links before filtering: %d", nrow(links_df))
...
required_link_cols <- c("peak", "gene", "score")
missing_link_cols <- setdiff(required_link_cols, names(links_dt))
if (length(missing_link_cols) > 0) {
  stop("Links object missing columns: ", paste(missing_link_cols, collapse = ", "))
}
...
baseline_df_full <- links_dt[, .SD[1], by = .(peak, gene)]
setorder(baseline_df_full, -score)

if (nrow(baseline_df_full) == 0) {
  stop("No LinkPeaks candidates survived candidate_filter=", candidate_filter)
}

msg("Number of peak-gene links after candidate_filter='%s': %d", candidate_filter, nrow(baseline_df_full))
...
msg("Scoring top %d baseline links after candidate filtering...", opt$candidate_top_k)
candidates <- head(baseline_df_full, opt$candidate_top_k)

if (nrow(candidates) == 0) {
  stop("No candidates available for coactivity scoring.")
}

msg("Candidate links selected for reranking: %d", nrow(candidates))
...
candidate_extra <- copy(candidates)
setnames(candidate_extra, "score", "link_score")
extra_link_cols <- setdiff(names(candidate_extra), c("peak", "gene", "link_score"))
if (length(extra_link_cols) > 0) {
  new_extra_names <- paste0("link_", extra_link_cols)
  setnames(candidate_extra, extra_link_cols, new_extra_names)
  features <- merge(
    features,
    candidate_extra[, c("peak", "gene", new_extra_names), with = FALSE],
    by = c("peak", "gene"),
    all.x = TRUE,
    sort = FALSE
  )
...
# Distance features
# ============================================================
msg("Adding nearest-transcript TSS distance features...")
gene_coord_df <- build_gene_tss_table()

features <- add_min_tss_distance(features, gene_coord_df, opt$distance_d0)
missing_gene_frac <- mean(is.na(features$gene_chr))
msg("Genes missing from TSS annotation in feature table: %.2f%%", 100 * missing_gene_frac)

baseline_dist <- add_min_tss_distance(baseline_df_full, gene_coord_df, opt$distance_d0)
baseline_missing_gene_frac <- mean(is.na(baseline_dist$gene_chr))
msg("Genes missing from TSS annotation in baseline table: %.2f%%", 100 * baseline_missing_gene_frac)
...
dropped_peak_frac <- if (n_peak_ids == 0) NA_real_ else 1 - length(peak_gr) / n_peak_ids
msg("Peaks retained for motif scoring: %d", length(peak_gr))
msg("Peaks dropped before motif scoring: %.2f%%", 100 * dropped_peak_frac)

if (length(peak_gr) == 0) {
  stop("No peaks retained for motif scoring after seqlevel/genome filtering.")
}
...
motif_base_names <- motif_names[tf_match$keep]
...
motif_names <- make.unique(paste0(motif_base_names, "|", motif_ids))
...
peak_summary <- data.table(
  ...
  peak_motif_score = unname(peak_best_motif_score),
  peak_tf_score = unname(peak_tf_score),
  peak_top_motifs = unname(peak_top_names)
)

features <- merge(features, peak_summary, by = "peak", all.x = TRUE, sort = FALSE)
features[is.na(peak_motif_score), peak_motif_score := 0]
features[is.na(peak_tf_score), peak_tf_score := 0]
features[is.na(peak_top_motifs), peak_top_motifs := NA_character_]

# Temporary compatibility aliases for the current evaluation script.
# New code should use the peak_* names above.
