...
option_list <- list(
  ...
  make_option(c("--score-mode"), type = "character", default = "full",
              help = "One of: linkpeaks, coactivity, coactivity_distance, coactivity_tf, distance_only, distance_mod_only, full, full_moddist, full_linkpeaks_anchored"),
...
calc_distance_modifier <- function(distance_score, lambda_distance) {
  # Symmetric distance modifier around distance_score = 0.5.
  # Nearby links are mildly boosted; distant links are mildly penalized.
  # Example: lambda=0.10 gives range approximately 0.95-1.05.
  1 + lambda_distance * (as.numeric(distance_score) - 0.5)
}
...
summarize_link_set <- function(dt, set_label, top_n = NA_integer_) {
  ...
  distance_modifier <- get_numeric_col(dt, "distance_modifier")
  ...
  data.table(
    ...
    median_distance_modifier = safe_median_num(distance_modifier),
...
write_validation_outputs <- function(ranked, output_dir, run_name) {
  ...
  inspection_cols <- intersect(c(
    ...
    "model_score", "link_score", "mul_weigh", "distance_score", "distance_modifier",
  ...
  component_cols <- intersect(c(
    "link_score", "mul_weigh", "distance_score", "distance_modifier", "distance_bp",
...
get_rank_table <- function(features, mode, lambda_distance, alpha_tf, high_q, medium_q) {
  ...
  } else if (mode == "distance_mod_only") {
    dt[, distance_modifier := calc_distance_modifier(distance_score, lambda_distance)]
    dt[, model_score := distance_modifier]
