...
For candidate pair `(p, g)` over cells `c \in C`, let `z^{RNA}_{gc}` and
`z^{ATAC}_{pc}` be the z-scored RNA expression of `g` and accessibility of `p`.

The coactivity score used throughout is `mul_weigh` (`run_linkpeaks_reranker.R:554`):

$$
A_{pg} \;=\; \frac{1}{|C|} \sum_{c \in C} \max\!\left(z^{RNA}_{gc},\, 0\right) \cdot \max\!\left(z^{ATAC}_{pc},\, 0\right)
$$
...
$$
\mathrm{add}_{pg} = \frac{1}{|C|}\sum_c \left[\max(z^{RNA}_{gc},0) + \max(z^{ATAC}_{pc},0)\right]
\qquad
\mathrm{mul}_{pg} = \frac{1}{|C|}\sum_c z^{RNA}_{gc} z^{ATAC}_{pc}
$$

$$
\mathrm{mul\_strict}_{pg} = \frac{1}{|C|}\sum_c \mathbb{1}\!\left[z^{RNA}_{gc} > 1\right]\mathbb{1}\!\left[z^{ATAC}_{pc} > 1\right]
\qquad
\mathrm{adj}_{pg} = \frac{\mathrm{mul\_strict}_{pg}}{\bar{r}_g \bar{a}_p + 10^{-6}}
$$

where `\bar{r}_g = \frac{1}{|C|}\sum_c \mathbb{1}[z^{RNA}_{gc} > 1]` and
...
$$
d_{pg} \;=\; \min_{t \in T(g)} \left| m_p - \mathrm{tss}_t \right|
$$

where `m_p` is the peak midpoint and `T(g)` the transcripts of `g`. The
...
so `D -> 1` at the TSS, `D = 0.5` at `d = d_0`, and `D approx 0.01` at 500 kb. The
...
$$
w_k \;=\; \mathrm{rescale}_{01}\!\left( \max_{j \in \mathcal{G}(k)} \; \overline{x}^{RNA}_{j} \right),
\qquad \overline{x}^{RNA}_{j} = \frac{1}{|C|}\sum_{c} x^{RNA}_{jc}
$$
...
`g_\alpha >= 1` and `f_\lambda > 0`, neither modifier can zero out a link — they
...
$$
f^{orig}_{\lambda}(D) \;=\; (1 - \lambda) + \lambda D
$$
...
$$
f^{mod}_{\lambda}(D) \;=\; 1 + \lambda\left(D - \tfrac{1}{2}\right)
$$
...
$$
f^{mod}_{\lambda}(D) \;=\; f^{orig}_{\lambda}(D) + \tfrac{\lambda}{2}
$$
...
| `distance_mod_only_lambda_0_1` | `distance_mod_only` | 0.10 | 0.00 | `f^{mod}_{0.1}(D_{pg})` | Control on the modified form |
| `coactivity_distance` | `coactivity_distance` | 0.30 | 0.00 | `A \cdot f^{orig}_{0.3}` | Add distance |
| `coactivity_tf` | `coactivity_tf` | 0.00 | 0.50 | `A \cdot (1 + 0.5\,T)` | Add TF/motif |
| `full` | `full` | 0.30 | 0.50 | `A \cdot f^{orig}_{0.3} \cdot (1 + 0.5\,T)` | Full, strong distance |
| `full_lambda_0_1` | `full` | 0.10 | 0.50 | `A \cdot f^{orig}_{0.1} \cdot (1 + 0.5\,T)` | Full, mild distance |
| `full_lambda_0_2` | `full` | 0.20 | 0.50 | `A \cdot f^{orig}_{0.2} \cdot (1 + 0.5\,T)` | Full, intermediate |
| `full_moddist_lambda_0_1` | `full_moddist` | 0.10 | 0.50 | `A \cdot f^{mod}_{0.1} \cdot (1 + 0.5\,T)` | Modified distance, mild |
| `full_moddist_lambda_0_2` | `full_moddist` | 0.20 | 0.50 | `A \cdot f^{mod}_{0.2} \cdot (1 + 0.5\,T)` | Modified distance, intermediate |
...
2. Retain genes and peaks with fraction `>=` `min_pair_frac: 0.02` (`:328–329`)
3. Same-chromosome cartesian join of gene TSS × peaks (`:353–358`)
4. Retain pairs with `\lvert m_p - \mathrm{tss}_g \rvert <=` `link_distance: 100000` (`:364–365`)
...
$$
\beta > 0 \quad\text{and}\quad p_{\mathrm{boot}} <= 0.05
$$
...
$$
\frac{\lvert p \cap p' \rvert}{\lvert p \rvert} >= 0.5
\quad\text{and}\quad
\frac{\lvert p \cap p' \rvert}{\lvert p' \rvert} >= 0.5
$$
...
Top-N support fractions are recomputed within `C_\delta` at
...
Filtering is method-independent — `C_\delta` is the same set for all methods —
...
11. **`f^{orig}_\lambda <= 1` always** (§9.1), so the original distance prior can
